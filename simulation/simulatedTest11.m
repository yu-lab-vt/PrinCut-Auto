...
%% generate data
sz=30;smRefLst=0.4:0.01:0.7;
smFactor=[1 1 1]*sz;
L=ceil(smFactor*max(smRefLst)*3)*2+1;
hrow = images.internal.createGaussianKernel(smFactor(1), ceil(L(1)+2.5*smFactor(1)));
hrow1=hrow(1:L(1));hrow2=hrow(end-L(1)+1:end);
...
dat1 = pagemtimes(hrow1*hcol,hslc);
dat2 = pagemtimes(hrow2*hcol,hslc);
dat=dat1+dat2;
dat=dat/max(dat(:));
...
title("2 cell gap, z=x/6, grow factor = "+num2str((1+HB)/(1+LB),'%.2f'));
axis([-inf inf 0.5 1]);
