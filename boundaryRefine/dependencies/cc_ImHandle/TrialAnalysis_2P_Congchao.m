%First seperate the 3 files into channels
% select the file path you want to analysis
filePath = '/Users/tpatriarchi1/Desktop/trials/';
...
for i=1:numel(files)
    ...
    % save channel 1 and channel 2 of one trial to traialName_stack 1 and trialName_stack 2
    ...
    %save different channels
    imwrite(FinalImage1(:,:,1), [filePath,FileTif,'_stack1.tif']);
    for k = 2:size(FinalImage1,3)
        imwrite(FinalImage1(:,:,k),  [filePath,FileTif,'_stack1.tif'], 'writemode', 'append');
    end
    
    imwrite(FinalImage2(:,:,1), [filePath,FileTif,'_stack2.tif']);
    for k = 2:size(FinalImage2,3)
        imwrite(FinalImage2(:,:,k),  [filePath,FileTif,'_stack2.tif'], 'writemode', 'append');
    end
